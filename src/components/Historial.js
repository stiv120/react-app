...
const Historial = () => {
  const [historial, setHistorial] = useState([]); // Inicializa como un arreglo vacío
  ...
  const getHistorial = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get("https://react-app-stiven.vercel.app/react-app/api/historial/ver");
  ...
  const historialFiltrado = historial.filter((item) => {
    ...
    return Object.values(item).some(
      valor => valor?.toString().toLowerCase().includes(textoBusqueda)
    );
  });
