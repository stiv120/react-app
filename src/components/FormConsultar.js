...
const FormConsultar = () => {
  ...
  // Implementar debounce para manejar las solicitudes a la API
  const debouncedFetchCities = useCallback(
    debounce(async (inputValue) => {
      if (inputValue.length > 2) {
        ...
        setOptions([]); // Limpiar opciones si el input es menor a 3 caracteres
      }
  ...
  const obtenerDatosApi = async (valorCiudad) => {
    if (valorCiudad) {
      try {
        ...
        const response = await axios.post("https://react-app-stiven.vercel.app/api/historial/guardar", JSON.stringify(datos), {
          headers: {
